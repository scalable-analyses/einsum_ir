...
namespace einsum_ir {
  namespace py {
    ...
    void Model::extract_primitive_dims( prim_t prim_main,
      ...
      for (size_t i = 0; i < dim_types.size(); i++) {
        if (exec_types[i] == exec_t::prim) {
          if (dim_types[i] == dim_t::m) {
            ...
            o_n = dim_sizes[i];
          } else if (dim_types[i] == dim_t::k && !is_brgemm) {
            o_k = dim_sizes[i];
          } else if (dim_types[i] == dim_t::k && is_brgemm) {
            o_br = dim_sizes[i];
            is_brgemm = false;
    ...
    void Model::extract_transpose_flags(std::vector<dim_t> const& dim_types,
                                        std::vector<exec_t> const& exec_types,
      ...
      for (size_t i = 0; i < dim_types.size(); i++) {
        if (exec_types[i] == exec_t::prim) {
          if (dim_types[i] == dim_t::k) {
            stride_a_k = strides_a[i];
          } else if (dim_types[i] == dim_t::n) {
            stride_b_n = strides_b[i];
          }
        }
      }
    ...
    int64_t Model::compute_gemm_iter( std::vector<exec_t> const& exec_types,
                                      std::vector<int64_t> const& dim_sizes) {
      int64_t gemm_iter = 1;
