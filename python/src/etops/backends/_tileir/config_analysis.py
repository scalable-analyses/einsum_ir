"""
Config analysis for the tileir backend.
...
class ConfigAnalysis:
    """Frozen analysis of a ``TensorOperationConfig`` for the tileir backend.
...
def analyze_config(config: TensorOperationConfig) -> ConfigAnalysis:
    """Analyze *config* and return a frozen :class:`ConfigAnalysis`.

    Args:
        config: A fully-specified ``TensorOperationConfig`` ready for
            compilation (all exec_types assigned, prim dims innermost).

    Returns:
        A frozen ``ConfigAnalysis`` with all derived values.

    Raises:
        ValueError: If *config* violates tileir constraints (e.g. no prim
            M/N/K, shared after seq, K shared, etc.).
    """
    ...
    num_shared = len(shared_loop_ids)
    num_seq = len(seq_loop_ids)
    ...
    def _build_tensor_view(
        ...
        Tuple[int, ...],  # shape
        Tuple[int, ...],  # strides
        Tuple[int, ...],  # tile_shape (1 for non-prim, size for prim)
        ...
        for vo_pos, cfg_idx in enumerate(view_order):
            ...
            if et == ExecType.prim:
                ...
                tile_shape.append(1)
                if et == ExecType.shared:
                    # position within shared_loop_ids
                    shared_pos = shared_loop_ids.index(cfg_idx)
                    index_map.append(("shared", shared_pos))
                elif et == ExecType.seq:
                    # offset within seq_loop_ids
                    seq_offset = seq_loop_ids.index(cfg_idx)
    ...
    shape_in0, strides_in0_tv, tile_in0, imap_in0 = _build_tensor_view(strides_in0)
    shape_in1, strides_in1_tv, tile_in1, imap_in1 = _build_tensor_view(strides_in1)
    shape_out, strides_out_tv, tile_out, imap_out = _build_tensor_view(strides_out)
...
def _validate(
    ...
    """Raise ``ValueError`` if *config* violates tileir constraints."""
    num_dims = len(dim_types)
