...
class einsum_ir::frontend::ResidualBlock {
  private:
    ...
    //! number of features in all occurring tensor
    int64_t m_num_features  = 0;
  ...
  public:
    ...
    //! splitting of the features to satisfy the required ones in the main kernel
    std::vector< int64_t > m_num_features_split;
    //! padding height
     ...
     * @param i_num_features number of features in the block.
     * @param i_stride strides used in the first convolution.
     * @param i_activations_0 input activations. assumed as (height+kernel_height-1, width+kernel_width-1, numfeatures). 
     ...
     * @param io_activations_1 intermediate activations. assumed to be zero-initialized as (height+kernel_height-1, width+kernel_width-1, numfeatures). 
     ...
     * @param io_activations_2 output activations to which the result is added to.  assumed as (height+kernel_height-1, width+kernel_width-1, numfeatures).
     **/
   ...
   *
   * @return SUCCESS if succesfull, error code otherwise. 
   **/
  err_t compile();
