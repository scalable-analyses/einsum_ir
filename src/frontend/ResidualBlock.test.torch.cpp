...
TEST_CASE( "Tests the generation of a residual blocks with strided convolutions and an increasing number of features.", "[residual_block]" ) {
  // allocate data
  int64_t l_width  = 96;
  int64_t l_height = 32;
  int64_t l_num_features_in = 128;
  int64_t l_num_features_out = 256;
  int64_t l_stride = 2;
  ...
  at::Tensor l_data_activation_0_no_pad = l_data_activation_0.narrow(        1, 1, l_height );
  l_data_activation_0_no_pad            = l_data_activation_0_no_pad.narrow( 2, 1, l_width );
  l_data_activation_0_no_pad.copy_( at::randn( {l_num_features_in, l_height, l_width} ) );
  ...
  l_data_activation_2_no_pad.copy_( at::randn( {l_num_features_out, l_height / l_stride, l_width / l_stride} ) );
