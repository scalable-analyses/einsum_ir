...
TEST_CASE( "Threading test for Iteration Space with SFC+OMP and 3 threads.", "[iter_space_sfc]" ) {
  ...
  for( int64_t l_thread_id = 0; l_thread_id < l_num_threads; l_thread_id++){
    ...
    l_iter.getInitialOffsets(l_thread_id, l_offset_left, l_offset_right, l_offset_out);
    ...
    for(int64_t l_id = 0; l_id < l_size; l_id++){
      l_iter.addMovementOffsets(l_thread_id, l_id, &l_ptr_left, &l_ptr_right, &l_ptr_out);
    }
  }
}
