"""CLI entry: ``python -m examples.spec.batched_gemm [--backend {tpp,blas}] [--schedule {scalar,scalar_reordered}] [--show]``."""

from __future__ import annotations

import argparse

from examples.spec.batched_gemm import DEFAULT_SCHEDULE, SCHEDULES, run


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the dba,dac->dbc batched GEMM example.",
    )
    parser.add_argument(
        "--backend",
        default="tpp",
